...
- 修正（用户明确：不需要压缩，内容详细完整优先、大小无限制）：撤销上一轮全部删空行操作，09/10/11 三章 18 篇行数还原到优化前（空行全保留）；仅保留两个有价值改动：09 章四篇目录补全 h3 子主题锚点（09.1 补 15 个/09.2 补 6 个/09.3 补 4 个/09.4 补 7 个）、11.2-DataStream API 删 AI 味词「最强大」。编写标准已同步更新：user profile + study-notes skill 顶部铁律「内容详细完整 > 一切，禁止压缩/删空行，大小无限制」
## [2026-08-19] 优化 | bigdata 09/10/11 三章 18 篇
- 09-Kafka与消息流：四篇目录补全 h3 子主题锚点（09.1 补 15 个、09.2 补 6 个、09.3 补 4 个、09.4 补 7 个，缩进子项格式对齐 07.2 样板）；删代码围栏内纯空行 16/7/15/18 行（零损耗），09.1 从 465 压回 449
- 10-Spark：六篇删代码围栏内纯空行 104/121/88/86/97/93 行（零损耗，内容完整保留），10.1 671→567、10.2 674→553、10.3 579→491、10.4 641→555、10.5 729→632、10.6 622→529；知识点全覆盖、代码可运行性抽查通过（scala/shell 示例专业完整）
- 11-Flink：11.2-DataStream API 删 AI 味词「最强大」（ProcessFunction 描述），其余七篇无 AI 味命中
- 检查结论：三章 frontmatter/围栏/wikilink 全合规，交叉引用 18 篇全部有效，无坏链；10 章超长系 Linux 模板 14 节结构（含常用命令/配置/故障排查/面试题等扩展节）所致，属内容密度而非冗余，按「内容完整优先」保留全部实质内容

## [2026-08-19] 新增 | java-fullstack 50-综合项目实战 新增 10 个落地项目
- 196-博客内容管理系统：CMS 实战（Redis 三级缓存 + ES 全文搜索 + 浏览量异步刷库），543 行
- 197-在线考试系统：题库管理/随机组卷/自动判分/切屏防作弊/限时，691 行
- 198-医院预约挂号系统：号源池定时放号/Redis 预扣不超卖/Redisson 锁/预约状态机/黑名单，663 行
- 199-酒店预订管理系统：房态日历/同房型同日期并发锁房/订单状态机/超时自动取消，728 行
- 200-短链接服务：DB 号段发号器/短码生成/302 跳转缓存/布隆过滤器防穿透/PV-UV 统计，937 行
- 201-任务协作看板系统：看板三级模型/拖拽状态流转/WebSocket 实时同步/乐观锁，939 行
- 202-会员积分与营销系统：积分流水一致性/防重领取/签到日历/ZSet 排行榜/过期清理，818 行
- 203-企业网盘系统：MinIO 对象存储/分片上传断点续传秒传/预签名 URL/分享链接/回收站，799 行
- 204-企业即时通讯系统：WebSocket 会话管理/单聊群聊/离线消息补拉/已读回执/在线状态，747 行
- 205-工单客服系统：状态机流转矩阵/SLA 超时提醒升级/技能组智能分派/满意度评价，818 行
- 每篇结构：项目背景(痛点+目标用户) → 功能介绍(按角色拆分) → 技术选型(表格) → 架构设计(分层图+链路) → 数据库设计(完整 DDL) → 核心实现(5-7 个可运行类) → 实际应用场景(3 个) → 最佳实践与踩坑记录(坑 6 条)
- 195 总览更新为 19 个项目（前 9 进阶路线 + 后 10 落地项目，各带 wikilink）；index.md 总文档数 1050→1060、Java 库 201→211 篇；README 目录树注释 9→19 个项目；总目录第五十篇补项目十~十九主题
- 编写执行新铁律：内容详细完整优先、大小无限制、不压缩（用户 2026-08-19 明确）

## [2026-08-19] 新增 | java-fullstack 50-综合项目实战 补齐项目七/八 + 再增 10 个落地项目
- 206-秒杀系统实战（补齐项目七）：Lua 原子扣库存防超卖、SETNX 限购、MQ 削峰、支付超时回补、IP 防刷，614 行，链 170-秒杀系统
- 207-数据平台实战（补齐项目八）：Kafka 管道、ES 按天索引+聚合、Redis 实时计数、HyperLogLog UV、大屏接口，654 行
- 208-电商后台管理系统：SPU/SKU 模型、库存联动、优惠券/满减促销，856 行
- 209-在线支付系统：支付宝/微信网关、回调验签、回调幂等、原路退款、日对账，936 行
- 210-物流配送管理系统：运单状态机、轨迹链、路由分派、异常件处理，852 行
- 211-招聘管理系统：简历解析、面试流程状态机、Offer 管理、人才库，894 行
- 212-企业知识库系统：文档树、版本回滚、ES 全文检索、审批发布，914 行
- 213-图书馆借阅管理系统：借阅状态机、逾期罚款、预约联动，881 行
- 214-停车场管理系统：阶梯计费规则引擎、防重复计费、月卡、道闸联动，983 行
- 215-问卷调查系统：动态题型模型、防重复提交、交叉分析、Excel 导出，905 行
- 216-会议室预约系统：冲突检测 SQL、并发防冲突、审批、纪要，830 行
- 217-外卖点餐系统：Redis 购物车、下单事务、订单状态机、配送，958 行
- 195 总览更新为 29 个项目（项目七/八替换为独立文档指引）；index.md 总文档数 1060→1072、Java 库 211→223 篇；README 注释 19→29；总目录补项目二十~二十九
- 12 篇全部：项目背景(痛点6条+目标用户) → 功能介绍(按角色) → 技术选型(表格) → 架构设计(分层图+链路) → 数据库设计(完整DDL+Redis Key) → 核心实现(5-7类) → 应用场景(3个) → 最佳实践+踩坑(6-7条)

## [2026-08-19] 新增 | java-fullstack 50-综合项目实战 再增 10 个创新项目（非 AI）
- 218-低代码表单引擎：JSON Schema 驱动 + 拖拽设计器 + schema_json/data_json 分离 + 后端权威校验 + 防重 + 模板套用，748 行（父代理手写样板）
- 219-可视化流程编排引擎：自研 DAG 执行引擎 + 条件分支求值 + 定时/事件触发 + 执行日志重试 + 版本管理（轻量自研非 Flowable），1126 行
- 220-多租户SaaS平台：租户隔离策略 + ThreadLocal 租户上下文 + 套餐订阅 + 用量计量 + 账单生成，920 行
- 221-实时协作白板：WebSocket + CRDT（OR-Set add-wins + LWW 寄存器）+ 光标同步 + 快照+日志回放 + 广播风暴控制，922 行
- 222-网约车调度系统：Redis GEO 就近匹配 + 派单策略 + 动态计价 + 行程状态机 + 司乘互评，1012 行
- 223-共享充电宝系统：租借归还状态机 + 归还计费 + 设备离线补偿 + 押金退还 + IoT 模拟，942 行
- 224-社区团购系统：成团判定 + 未成团自动退款 + 团长佣金分账 + 自提点管理，1031 行
- 225-电子合同签署系统：RSA 数字签名 + 时间戳 + PDF 坐标签章 + 哈希链防篡改 + 验真（无 CA 可演示），1247 行
- 226-统一认证中心：OAuth2 授权码 + SSO 全局/局部会话 + JWT + Token 黑名单 + 跨域登录态，1058 行
- 227-定时任务调度平台：Quartz 风格 + Redis 锁防重复执行 + 执行日志链路 + 失败重试 + DAG 编排 + 钉钉/邮件告警，1189 行
- 195 总览更新为 39 个项目（项目三十~三十九 带 wikilink）；index.md 总文档数 1072→1082、Java 库 223→233 篇；README 注释 29→39；总目录补项目三十~三十九
- 10 篇全部：项目背景(痛点6条+目标用户) → 功能介绍(按角色) → 技术选型(表格) → 架构设计(分层图+链路) → 数据库设计(完整DDL+Redis Key) → 核心实现(5-7类) → 应用场景(3个) → 最佳实践+踩坑(6-7条)，围栏偶数、wikilink 全有效

## [2026-08-20] create | 微信小程序与后台知识库

- 新建《微信小程序与后台学习知识库总目录.md》：24 篇章、106 个主题
- 搭建 `weapp-fullstack/` 知识库骨架（24 章节目录 + 106 骨架文档 + README 进度表）
- 覆盖：小程序注册/基础/WXML/WXSS/JS/组件/API/登录支付/订阅消息/分享/分包性能/安全合规/测试/发布上线 + SpringBoot 后台 + Vue 管理后台 + 联调部署 + 全栈项目实战 + 面试
- 已完成 01.1-小程序概述与学习路线（样板），其余篇章分批委派编写中

## [2026-08-20] create | 微信小程序与后台知识库 106 篇全部完成

- 8 批并行委派（24 个子代理）完成全部篇章编写，补写超时漏写的 07.4-组件进阶、12.4-广告与商业化
...
## [2026-09-20] create | Kafka 完整教程（4.1.2 / KRaft，含 7 个可运行脚本与 2 个 Maven 示例工程）

- kafka/Kafka完整教程.md（2713 行，type: concept）：Kafka 完整教程——概念与 KRaft 架构（broker/controller 角色、分区与副本、ISR 与 ELR、segment 与索引、消费组协调、数据目录与四个 checkpoint 文件）、部署（apache/kafka:4.1.2 容器单节点实测、二进制包 + systemd 裸机部署实测、三节点集群的可行拓扑、关键配置逐条解释、advertised.listeners 与内部主题副本因子两个高频坑）、命令行工具全解（主题管理、收发消息、消费组与 LAG、位移重置、配置管理与 synonyms、dump-log 消息批次结构、log-dirs、perf-test 实测吞吐）、存储格式与保留策略（三段式查找、消息批次字段含义、按时间/大小清理实测、compact 与 tombstone、容量规划与磁盘满的症状）、生产者（同步/异步/key 分区/显式分区/参数表/事务生产者，原生 Java 客户端实跑）、消费者与消费组（Rebalance、手动提交三种方式、并发与分区数、常见异常）、可靠性（不丢不重有序的配置矩阵、三副本集群三次宕机演练实测：Leader 切换、ISR 收缩、NotEnoughReplicasException、ISR 回补）、集群运维（副本分布与首选 Leader、KRaft quorum 状态、分区重分配实测、监控指标清单、规模与规格参考）、Spring Boot 2.7 集成（KafkaTemplate、@KafkaListener、手动 ack、DefaultErrorHandler + 死信主题闭环，实跑 8 条事件 7 成功 1 进死信）、生态与选型对比（Connect/Streams/ksqlDB/Schema Registry，与 RabbitMQ/RocketMQ/Pulsar 对比）、3 个应用场景（订单事件流、日志削峰与积压追平、CDC 与事务原子链路）、17 条最佳实践与 15 条踩坑记录、3.x 与 4.x 差异与迁移清单
- 配套可运行资产（kafka/examples/）：scripts/01-single-up.sh（启动与主题管理）、02-produce-consume.sh（收发/消费组/位移/保留/segment/性能）、03c-cluster.sh（单 controller + 三 broker 集群：副本、ISR、宕机演练、分区重分配）、04-native-systemd.sh（裸机 + systemd + JMX）、05-retention.sh（消息过期）、06-scenario-backlog.sh（削峰与积压追平）；java/kafka-client-demo（TopicAdmin/ProducerDemo/ConsumerDemo/TransactionDemo，Maven + Java 8 目标）、java/springboot-kafka-demo（Spring Boot 2.7.18 + spring-kafka 2.8.11，手动 ack + 重试 + DLT）
